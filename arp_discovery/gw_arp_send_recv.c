...
int main()
{
        ...
        unsigned char source_ip[4] = {10,0,0,35};
        unsigned char target_ip[4] = {10,0,0,10};
	char null_mac=0, MacId[32]="", *MacIdptr;
        struct ifreq ifr;
                        ...
                        for(index=0;index<6;index++)
			{
				if (arp_resp->target_mac[index] == 0x00)
					null_mac=null_mac+1;
                                ...
                                //strcat(MacId, (char *)arp_resp->target_mac[index]);
				MacIdptr+=sprintf(MacIdptr,"%02x:", arp_resp->target_mac[index]);
			}

                        printf("\n  :");
			if (null_mac >= 5)
				printf("No response : failed:%d\n",null_mac); 
			else
				printf("Mac ID str: %s\n",MacId);
                        break;
                }
