...
int Create_ListenerForcamera(){
	...
	while(1){
		...
		IPCTempBufIndex=0;
		memset(IPCTempBuffer, 0, maxlen);
		readfd = accept (sock, (struct sockaddr *) &server_address, &addlen);
		if(readfd != -1){

			while ((BuffSize = recv(readfd, RecBuffer, 1500, 0)) > 0) {
			...
			if(RecvComplete==1)
			{
				...
				if(tmp != NULL){
					...
					tmp += 4 ;	//	Ignore "\r\n\r\n"
					//printf("tmp3[%s]\n",tmp);
					sprintf (FileName, "ServerImage%d.jpg",Index) ;
